...
function validateCred(array) {
  ...
  const sum = slicedArray.reduce(add) + checkDigit;
...
function idInvalidCardCompanies(batch) {
  ...
  for(let i = 0; i < batch.length; i++) {
    if(validateCred(batch[i]) === false) {
      if(batch[i][0] === 3) {
        if(invalidCompanies.includes('Amex') === false) {
          invalidCompanies.push('Amex');
        }
      } else if(batch[i][0] === 4) {
        if(invalidCompanies.includes('Visa') === false) {
          invalidCompanies.push('Visa');
        }
      } else if(batch[i][0] === 5) {
        if(invalidCompanies.includes('Mastercard') === false) {
          invalidCompanies.push('Mastercard');
        }
      } else if(batch[i][0] === 6) {
        if(invalidCompanies.includes('Discover') === false) {
